...
final class WebRemoteServer: @unchecked Sendable {
    ...
    func start() throws {
        ...
        // Keep the service type stable so already paired phones continue to
        // discover the app after the user-facing Photo Sift rebrand.
        let service = NetService(domain: "local.", type: "_cr3cleaner._tcp.", name: "Photo Sift", port: port)
...
<!doctype html><html><head><meta name="viewport" content="width=device-width,initial-scale=1"><title>Photo Sift Remote</title>
...
<body><form class="box" method="post" action="/login"><h2>Photo Sift</h2><p>Enter the pairing PIN shown on your Mac. This phone stays paired for future launches.</p><p class="error">{{ERROR}}</p><input name="pin" inputmode="numeric" pattern="[0-9]{6}" maxlength="6" autofocus required placeholder="6-digit PIN"><button>Connect</button></form></body></html>
"""#

    static let appHTML = #"""
<!doctype html><html><head><meta name="viewport" content="width=device-width,initial-scale=1,maximum-scale=1,user-scalable=no,viewport-fit=cover"><title>Photo Sift Remote</title>
<style>
...
.grid{gap:4px}.card img,.rawRow img,.filmstrip img{object-fit:contain}.grid.browse{gap:3px;padding:3px 3px 100px;grid-template-columns:repeat(auto-fill,minmax(112px,1fr))}.grid.browse .card{border-radius:3px;background:#050505}.grid.browse .card img{width:100%;height:auto;max-height:none;aspect-ratio:auto;object-fit:contain}.grid.browse .meta{padding:5px}.grid.browse .issue,.grid.browse .group{display:none}.previewStage{touch-action:none}.previewCanvas{width:100%;height:100%;display:grid;place-items:center;transform-origin:center;will-change:transform}.previewCanvas img{grid-area:1/1;width:auto;height:auto;max-width:100%;max-height:100%;object-fit:contain}.previewCanvas img.low{width:100%;height:100%}
</style></head><body>
<div class="shell"><header><div class="top"><div class="brand"><div class="logo">◫</div><div><div class="eyebrow">Private Mac Remote</div><h1>Photo Sift</h1></div></div><b class="folderPill" id="folder">No folder</b></div><div class="status" id="status">Connecting…</div><div class="summary"><div class="summaryCard"><b id="photoStat">—</b><span>Browse photos</span></div><div class="summaryCard"><b id="rawStat">—</b><span>Orphaned RAW</span></div><div class="summaryCard"><b id="backupStat">—</b><span>Need backup</span></div></div><nav class="tabs"><button id="reviewTab" class="active">Photos</button><button id="rawTab">RAW Companion</button><button id="backupTab">Backup</button></nav><div class="folderChooser"><button id="upFolder" aria-label="Parent folder">↑ Up</button><button id="browseFolders">Browse folders…</button></div></header>
<section id="reviewPanel"><div class="panelHead"><select id="filter" aria-label="Photo filter"><option value="browse">Browse all photos (no AI)</option><option value="all">All review flags</option><option value="strongBlur">Strong blur (75%+)</option><option value="anyBlur">Any blur</option><option value="blink">Closed eye / blink</option><option value="faceQuality">Low face quality</option><option value="underexposed">Underexposed</option><option value="overexposed">Overexposed</option><option value="lowResolution">Low resolution</option><option value="duplicate">Exact duplicates</option><option value="burstBest">Best of each burst</option><option value="bursts">All burst photos</option></select><button id="browse">Refresh Photos</button><label><input type="checkbox" id="dry"> Dry Run</label><button class="primary" id="analyze">Analyze Photos</button><button id="undo">Undo</button></div><main id="grid" class="grid browse"></main></section>
...
function render(){renderReview();renderRaw();renderBackup();renderFolders();syncViewer()}
...
function syncViewer(){if(!$('#viewer').classList.contains('open'))return;const isRaw=viewerMode==='raw',items=isRaw?rawShown:shown;if(!items.length){$('#viewer').classList.remove('open');return}const previous=isRaw?rawCurrent:current,index=items.findIndex(item=>item.id===viewerItemID),next=index>=0?index:Math.min(previous,items.length-1);renderFilmstrip(items,next,isRaw);selectViewer(next,isRaw,false)}
function openViewer(i){resetZoom();$('#viewer').classList.add('open');renderFilmstrip(shown,i,false);selectViewer(i,false,false);requestAnimationFrame(()=>$('#filmstrip button.active')?.scrollIntoView({behavior:'auto',inline:'center',block:'nearest'}))}
function openRawViewer(i){resetZoom();$('#viewer').classList.add('open');renderFilmstrip(rawShown,i,true);selectViewer(i,true,false);requestAnimationFrame(()=>$('#filmstrip button.active')?.scrollIntoView({behavior:'auto',inline:'center',block:'nearest'}))}
...
async function waitForRemoval(id,isRaw){for(let i=0;i<8;i++){if(!(isRaw?rawShown:shown).some(item=>item.id===id))return;await new Promise(resolve=>setTimeout(resolve,150));await refresh()}}
async function trashCurrent(){const p=shown[current];if(!p)return;const nextID=shown.length>1?shown[(current+1)%shown.length].id:null,dryRun=state.dryRun;if(!confirm(`${dryRun?'Dry Run: review':'Move to Trash'} ${p.name}?`))return;await api('/api/trash',{ids:[p.id]});if(!dryRun)await waitForRemoval(p.id,false);if(nextID){const i=shown.findIndex(photo=>photo.id===nextID);if(i>=0)selectViewer(i,false,true)}else if(!dryRun)$('#viewer').classList.remove('open')}
async function trashRawCurrent(){const p=rawShown[rawCurrent];if(!p)return;const nextID=rawShown.length>1?rawShown[(rawCurrent+1)%rawShown.length].id:null,dryRun=state.dryRun;const question=`${dryRun?'Dry Run: review':'Move to Trash'} 1 CR3${p.xmpCount?' + '+p.xmpCount+' XMP':''} (${formatBytes(p.byteCount)})? JPG files will not be changed.`;if(!confirm(question))return;await api('/api/trash-raw',{ids:[p.id]});if(!dryRun)await waitForRemoval(p.id,true);if(nextID){const i=rawShown.findIndex(file=>file.id===nextID);if(i>=0)selectViewer(i,true,true)}else if(!dryRun)$('#viewer').classList.remove('open')}
...
$('#previewStage').ontouchstart=e=>{tapMoved=false;if(e.touches.length===2){wasPinching=true;pinchStart=touchDistance(e.touches);pinchScale=zoomScale;panStart=null}else if(e.touches.length===1&&zoomScale>1){panStart={x:e.touches[0].clientX,y:e.touches[0].clientY,ox:zoomX,oy:zoomY}}};$('#previewStage').ontouchmove=e=>{if(e.touches.length===2&&pinchStart){e.preventDefault();tapMoved=true;zoomScale=Math.max(1,Math.min(5,pinchScale*touchDistance(e.touches)/pinchStart));if(zoomScale===1)zoomX=zoomY=0;applyZoom()}else if(e.touches.length===1&&panStart&&zoomScale>1){e.preventDefault();const dx=e.touches[0].clientX-panStart.x,dy=e.touches[0].clientY-panStart.y;if(Math.hypot(dx,dy)>8)tapMoved=true;zoomX=panStart.ox+dx;zoomY=panStart.oy+dy;applyZoom()}};$('#previewStage').ontouchend=e=>{const now=Date.now();if(!tapMoved&&zoomScale>1&&e.changedTouches.length===1){if(now-lastTap<300){resetZoom();lastTap=0;wasPinching=true}else lastTap=now}pinchStart=0;panStart=null;if(zoomScale<1.03)resetZoom();setTimeout(()=>wasPinching=false,0)};
